...
// Program also extracts digital inputs from .bin file and puts them into .digbin file. Optionally, DI channes can be included into the .dat file to make up channels 64 - 79.
...
main (int argc, char *argv[]) {
  ...
  struct CsicsvariDat { short int data[192*2]; } datblock[BLOCKSIZE / 2];   /*sizeof(short int) is 2 bytes*/
  ...
  // !!!
  fread(&axraw,size_packet,1,fpin);
  ...
  while((blocksread = fread(&axraw,size_packet,BLOCKSIZE,fpin)) == BLOCKSIZE) {
	...
	{
	  first_packet_no = axraw[i].n/2;
	}

      if(axraw[i].id[3] == '2' || axraw[i].id[3] == 'B' || axraw[i].id[3] == 'F') // If the current block contains tracking data
	{
	  tracking_counter++;

	  fprintf(fpouttrk,"%u\t%u\t%u\t%hd\t%hd\t%hd\t%hd\t%hd\t%hd\t%hd\t%hd\n",
		 ((axraw[i].n/2-first_packet_no)*3+1),
		 axraw[i].n/2,
      ...
      if(outdat==1)    // prepare digital input structures
	{
	  ...
	  if((axraw[i].digio.di_01 & 1)==1) digData[0] = 10000;
	  else if((axraw[i].digio.di_02 & 1)==1) digData[1] = 10000;
	  else if((axraw[i].digio.di_03 & 1)==1) digData[2] = 10000;
	  else if((axraw[i].digio.di_04 & 1)==1) digData[3] = 10000;
	  else if((axraw[i].digio.di_05 & 1)==1) digData[4] = 10000;
	  else if((axraw[i].digio.di_06 & 1)==1) digData[5] = 10000;
	  else if((axraw[i].digio.di_07 & 1)==1) digData[6] = 10000;
	  else if((axraw[i].digio.di_08 & 1)==1) digData[7] = 10000;
	  else if((axraw[i].digio.di_09 & 1)==1) digData[8] = 10000;
	  else if((axraw[i].digio.di_10 & 1)==1) digData[9] = 10000;
	  else if((axraw[i].digio.di_11 & 1)==1) digData[10] = 10000;
	  else if((axraw[i].digio.di_12 & 1)==1) digData[11] = 10000;
	  else if((axraw[i].digio.di_13 & 1)==1) digData[12] = 10000;
	  else if((axraw[i].digio.di_14 & 1)==1) digData[13] = 10000;
	  else if((axraw[i].digio.di_15 & 1)==1) digData[14] = 10000;
	  else if((axraw[i].digio.di_16 & 1)==1) digData[15] = 10000;
	}
	...
	for (j=0;j<192;j++) /*sort samples in *current packet**/
	  {
	    int add = (first_half == 1) ? 0 : 64;
	    if (j >= 128){
		add += 128;
	    } else if (j >= 64){
		add += 64;
	    }
	    datblock[i/2].data[j + add] = axraw[i].data[remap[j]]+1; // sort original data into correct probe sequence
	  }
	...
	if(diindat==0){
		if (first_half == 0){
			fwrite(datblock,size_out,BLOCKSIZE/2,fpout);  //write datblock array
		}
	}
	else fwrite(extended_datblock,size_out_extended,BLOCKSIZE/2,fpout);
  ...
  for(i=0;i<blocksread;i++) { /* loop thru all packets read into memory*/
	...
	for (di_channel_counter=0;di_channel_counter<16;di_channel_counter++) digData[di_channel_counter] = 0;    //fill entire array with zeros
	// find those digital input channels that are at 1 and fill them into the output array
	if((axraw[i].digio.di_01 & 1)==1) digData[1] = 10000;
	else if((axraw[i].digio.di_02 & 1)==1) digData[2] = 10000;
	else if((axraw[i].digio.di_03 & 1)==1) digData[3] = 10000;
	else if((axraw[i].digio.di_04 & 1)==1) digData[4] = 10000;
	else if((axraw[i].digio.di_05 & 1)==1) digData[5] = 10000;
	else if((axraw[i].digio.di_06 & 1)==1) digData[6] = 10000;
	else if((axraw[i].digio.di_07 & 1)==1) digData[7] = 10000;
	else if((axraw[i].digio.di_08 & 1)==1) digData[8] = 10000;
	else if((axraw[i].digio.di_09 & 1)==1) digData[9] = 10000;
	else if((axraw[i].digio.di_10 & 1)==1) digData[10] = 10000;
	else if((axraw[i].digio.di_11 & 1)==1) digData[11] = 10000;
	else if((axraw[i].digio.di_12 & 1)==1) digData[12] = 10000;
	else if((axraw[i].digio.di_13 & 1)==1) digData[13] = 10000;
	else if((axraw[i].digio.di_14 & 1)==1) digData[14] = 10000;
	else if((axraw[i].digio.di_15 & 1)==1) digData[15] = 10000;
	else if((axraw[i].digio.di_16 & 1)==1) digData[16] = 10000;
      }
    ...
    // Now, axraw[i].data[0 through 191] contains 3 blocks of 64 samples. digData[0 through 15] contains 16 DI channels.
    //printf("i: %d\n",i);
    if(diindat==1)
      {
	//printf("writing new extended_data_array\n");
  ...
  if(outdat==1)
    {
      if(diindat==0) fwrite(datblock,size_out,blocksread/2,fpout);  //write datblock array
  ...
  minutes=(int)((n*3)/samprate/120.0);
  seconds=((n*3)/samprate/2.0-(minutes*60));
