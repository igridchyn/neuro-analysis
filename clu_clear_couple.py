#!/usr/bin/env python2
# -*- coding: utf-8 -*-

import numpy as np
import scipy
import os
from sys import argv
import matplotlib.pylab as plt
from scipy import linalg
from scipy import spatial
import subprocess
import shutil
import struct

def printProgressBar (iteration, total, prefix = '', suffix = '', decimals = 1, length = 100, fill = '█'):
    """
    Call in a loop to create terminal progress bar
    @params:
        iteration   - Required  : current iteration (Int)
        total       - Required  : total iterations (Int)
        prefix      - Optional  : prefix string (Str)
        suffix      - Optional  : suffix string (Str)
        decimals    - Optional  : positive number of decimals in percent complete (Int)
        length      - Optional  : character length of bar (Int)
        fill        - Optional  : bar fill character (Str)
    """
    percent = ("{0:." + str(decimals) + "f}").format(100 * (iteration / float(total)))
    filledLength = int(length * iteration // total) + 1
    bar = fill * filledLength + '-' * (length - filledLength)
    print '\r%s |%s| %s%% %s' % (prefix, bar, percent, suffix),
    # Print New Line on Complete
    if iteration == total:
        print ' '
#=================================================================================================================
def onclick(event):
	print 'button=%d, x=%d, y=%d, xdata=%f, ydata=%f' % (
		event.button, event.x, event.y, event.xdata, event.ydata)
	if event.button == 2:
		print 'delete %.2f percent of closest to cluster %d spikes from cluster %d...' % (event.xdata, clun1, clun2)
		# round down to closest xval
                for i in range(len(xvals)):
                    if xvals[i+1] > event.xdata:
                        i = xvals[i]
                        break
		#i = round(event.xdata)	
		print '	step = ', step
		print '	i= ', i
		print 'len(imsort)= ', len(imsort)
		res_cut = scipy.array([res2[j] for j in imsort[0:(int(i/100.0 * len(imsort)))]])
		clean_cluster(res_cut)
		print 'done :' + str(len(res_cut)) + ' spikes move to cluster ' + str(clun1)
		exit(0)
#=================================================================================================================
def ref_frac(rescut, ref_ms_1, ref_ms_2):
	diff = scipy.array([rescut[i] - rescut[i-1] for i in range(1, len(rescut))])
	i2 = sum(diff < 20 * ref_ms_1)
	i10 = sum(diff < 20 * ref_ms_2)
	return i2 / float(i10) if i10 > 0 else 0, i2, i10
#=================================================================================================================
def isi_upd(isi, ref_ms_1, ref_ms_2, i1, i2):
	if isi < 20 * ref_ms_1:
		i1 += 1
	if isi < 20 * ref_ms_2:
		i2 += 1
	
	return (i1, i2)
#=================================================================================================================
def isi_upd_rem(isi, ref_ms_1, ref_ms_2, i1, i2):
	if isi < 20 * ref_ms_1:
		i1 -= 1
	if isi < 20 * ref_ms_2:
		i2 -= 1
	
	return (i1, i2)
#=================================================================================================================
#=================================================================================================================
def ref_frac_inc(rescut, rescutnew, ref_ms_1, ref_ms_2, i1, i2):
	# both res are sorted
	rescutm = [0] * (len(rescut) + len(rescutnew))
	m = 0

	# print rescut, rescutnew

	if len(rescutnew) == 0:
		frac = i1 / float(i2) if i2 > 0 else 0
		return rescut[:], i1, i2, frac

	i = 1
	j = 0

	# find first between rescut[0] and rescut[1]
	while j < len(rescutnew) and rescutnew[j] < rescut[0]:	
		# 2 new ISIs:
		(i1, i2) = isi_upd(rescut[0] - rescutnew[j], ref_ms_1, ref_ms_2, i1, i2)
		rescutm[m] = rescutnew[j]
		m += 1
		j += 1

	rescutm[m] = rescut[0]
	m += 1

	while i < len(rescut) and j < len(rescutnew):
		while i < len(rescut) and rescut[i] < rescutnew[j]:
			rescutm[m] = rescut[i]
			m += 1
			i += 1

		if i == len(rescut):
			break

		# 2 new ISIs:
		(i1, i2) = isi_upd(rescutnew[j] - rescut[i-1], ref_ms_1, ref_ms_2, i1, i2)
		(i1, i2) = isi_upd(rescut[i] - rescutnew[j], ref_ms_1, ref_ms_2, i1, i2)

		rescutm[m] = rescutnew[j]
		m += 1

		j += 1

	# assigne the rest
	if i >= len(rescut):
		rescutm[m:] = rescutnew[j:]
	else:
		rescutm[m:] = rescut[i:]

	frac = i1 / float(i2) if i2 > 0 else 0
	return rescutm, i1, i2, frac
#=================================================================================================================
def ref_frac_inc_rem(rescut, rescutrem, ref_ms_1, ref_ms_2, i1, i2):
	# both res are sorted
	# print rescut, rescutrem

        # ? remove rescutrem from rescut ?

	rescutm = [0] * (len(rescut) - len(rescutrem))
	m = 0;

	if len(rescutrem) == 0:
		frac = i1 / float(i2) if i2 > 0 else 0
		return rescut, i1, i2, frac

	i = 0
	j = 0
	# find first not excluded
	while i+1< len(rescut) and i < len(rescutrem) and rescut[i] == rescutrem[j]:
		(i1, i2) = isi_upd_rem(rescut[i+1] - rescut[i], ref_ms_1, ref_ms_2, i1, i2)	
		i += 1
		j += 1
	
	while i < len(rescut) and j < len(rescutrem):
		while i < len(rescut) and rescut[i] < rescutrem[j]:
			# if m == len(rescutm):
			# 	print m, len(rescutm), i, rescut[i]
			rescutm[m] = rescut[i]
			m += 1
			i += 1

		if i == len(rescut):
			break

		# 2 new ISIs, now rescut[i] == rescutrem[j]
		# if previous has not been removed (and isi already discarded)
		if j == 0 or rescutrem[j-1] != rescut[i-1]:
			(i1, i2) = isi_upd_rem(rescut[i] - rescut[i-1], ref_ms_1, ref_ms_2, i1, i2)
		# TODO: update for case when >1 in a row have been removed (keep last non-removed and check if next has not been removed)
		if i+1 < len(rescut):
			(i1, i2) = isi_upd_rem(rescut[i+1] - rescut[i], ref_ms_1, ref_ms_2, i1, i2)

			# find previous not-removed and udpate if next is not removed
			k = i - 1
			l = j - 1
			while k >=0 and l >= 0 and rescut[k] == rescutrem[l]:
				k -= 1
				l -= 1
			if k >= 0 and (j + 1 == len(rescutrem) or rescutrem[j+1] != rescut[i+1]):
				(i1, i2) = isi_upd(rescut[i+1] - rescut[k], ref_ms_1, ref_ms_2, i1, i2)

		j += 1
		i += 1

	# print m,len(rescutm), i, len(rescut)
	# append the rest of the res
	rescutm[m:] = rescut[i:]

	frac = i1 / float(i2) if i2 > 0 else 0
	return rescutm, i1, i2, frac
#=================================================================================================================
#=================================================================================================================
def clean_cluster(res_cut1):
    ss_cumul = 0
    i = 0
    irescut = 0
    for path in BASES:
	fclu = open(path + '.clu')
	fclunew = open(path + '.clu.tmp', 'w')
	#fclunew.write(str(nclu + 1) + '\n')
	#fclu.readline()

        fresnew = open(path + '.res.tmp', 'w')

	# print res_cut1

	for line in fclu.readlines():
		c = int(line[:-1])
		# print c, res[i]
                # ??? stronger comparison: res_cut1[irescut] == res[i] ???
		if c == clun2 and res[i] in res_cut1:
			fclunew.write(str(NEWID) + '\n')
                        irescut += 1
		else:
                        if c >= NEWID:
                            fclunew.write(str(int(line) + 1) + '\n')
                        else:
        		    fclunew.write(line)

        

                fresnew.write(str(res[i]-ss_cumul) + '\n')
		i += 1

                # write entries in res_cut1 that are not in res (for unclustered case)
                if clun2 == 0:
                    while irescut < len(res_cut1) and i < len(res) and res_cut1[irescut] < res[i]:
                        fclunew.write(str(nclu + 1) + '\n')
                        fresnew.write(str(res_cut1[irescut]-ss_cumul) + '\n')
                        irescut += 1

        fss = open(os.path.dirname(path) + '/session_shifts.txt')
        ss = int(fss.read())
        ss_cumul += ss

        fclushinew = open(path + '.cluster_shifts.tmp', 'w')
        if int(tetr) < len(c_shifts) - 1:
            for t in range(int(tetr)+1, len(c_shifts)):
                c_shifts[t] += 1
        fclushinew.write('\n'.join([str(c) for c  in c_shifts]))

	fclu.close()
	fclunew.close()
        fresnew.close()
        fclushinew.close() 

        # DEBUG
        shutil.move(path+ '.clu.tmp', path + '.clu')
	shutil.move(path+ '.cluster_shifts.tmp', path + '.cluster_shifts')
#=================================================================================================================

if len(argv) < 7:
	print 'Output cluster purity from number of spikes removed from the overlap of the two clusters'
	print 'Arguments: (1)<number of features> (2)<tetrode> (3)<refractory window size in ms, single-sided> (4)<step in %> (5)<limit % for transfer calculation> (6-N)<bases>'
	exit()

#path = argv[1]
NFET = int(argv[1])
tetr = argv[2]
#clun1 = int(argv[3])
#clun2 = int(argv[4])
refr_ms_1 = int(argv[3])
STEP = int(argv[4])
LIMPER = float(argv[5])
BASES = argv[6:]




fets = []
ts = []
ss_cum = 0
for base in BASES:
    f = open(base + '.fetb' + '.' + str(tetr))
    # N float features, 4 float special features, int time

    fmt = (NFET + 4) * 'f' + 'i'
    spike_size = 4 * (NFET + 4 + 1)

    while True:
        spikebin = f.read(spike_size)

        if not spikebin:
            break

        fet = struct.unpack(fmt, spikebin)
        fet = np.array(fet)
        fets.append(fet[:NFET])
        ts.append(int(fet[-1]) + ss_cum)

    # update sessions shift
    fss = open(os.path.dirname(base) + '/session_shifts.txt')
    ss_cum += int(fss.read())

    print 'Cumulative session shift:', ss_cum

fet = np.array(fets)

print fet.shape

# build map time -> features
mtf = {}
for i in range(len(ts)):
    mtf[ts[i]] = fet[i,:]

print 'Fet times:', ts[:10]
print 'Dic size (t->fet)', len(mtf)


#f = open(path + '.fet.' + tetr)
#dim = int(f.readline()[:-1])
#x = scipy.fromfile(f, sep=' ')
#x = x.reshape(x.shape[0] / dim, dim)
#f.close()

#fetdim = dim - 5
fetdim = NFET

#i = scipy.array(range(0, fetdim))
#x = x[:, i]

clu = []
res = []
# cumnulative session shift
ss_cum = 0
for path in BASES:
    fclu = open(path + '.clu')
    #nclu = int(fclu.readline()[:-1])
    clup = fclu.read().split('\n')
    clup = [int(c) for c in clup if len(c) > 0]
    clu.extend(clup)
    fclu.close()

    fres = open(path + '.res')
    resp = fres.read().split('\n')
    resp = [int(r) + ss_cum for r in resp if len(r) > 0]
    res.extend(resp)
    fres.close()

    # update sessions shift
    f = open(os.path.dirname(path) + '/session_shifts.txt')
    ss_cum += int(f.read())

nclu = max(clu)


# TODO: find out from cluster_shifts
NEWID = 0
fcs = open(BASES[0] + '.cluster_shifts')
c_shifts = [int(cs) for cs in fcs if len(cs) > 0]
if int(tetr) == len(c_shifts) - 1:
    NEWID = max(clu) + 1
else:
    NEWID = c_shifts[int(tetr) + 1] + 1


print 'Res', res[:10]

while True:
    print '\n'
    sclu = raw_input('Enter numbers of clusers: ')
    if len(sclu) < 3:
        exit(0)
    if sclu == 'par':
        print 'LIMPER(=%.2f %%) NSTEP(=%d steps) REFRAC(=%d ms)' % (LIMPER, STEP, refr_ms_1)
        continue
    if sclu == 'LIMPER':
        LIMPER = float(raw_input('Enter new parameter value: '))
        continue
    if sclu == 'NSTEP':
        STEP = int(raw_input('Enter new parameter value: '))
        continue

    wsclu = sclu.split(' ')
    clun1 = int(wsclu[0])
    clun2 = int(wsclu[1])

    # indices of cluster 1 elements
    ind1 = scipy.array([i for i in range(0, len(clu)) if clu[i] == clun1])
    res1 = [res[i] for i in ind1.tolist()]
    fet1 = np.array([mtf[r] for r in res1 if r in mtf])

    # indices of cluster 2 elements
    ind2 = scipy.array([i for i in range(0, len(clu)) if clu[i] == clun2])
    res2 = [res[i] for i in ind2.tolist()]
    fet2 = np.array([mtf[r] for r in res2 if r in mtf])
    #fet2 = x[ind2, :]
    # number of elements in cluster 2
    n2 = ind2.shape[0]

    # FOR 0 CLUSTER - unclustered spikes, therefore in fet but not in res
    if clun2 == 0:
        res2 = [t for t in ts if t not in res]
        fet2 = np.array([mtf[r] for r in res2 if r in mtf])
        n2 = len(res2)

    print 'Numbers of spikes in two clusters:', len(res1), len(res2)
    print 'Numbers of spikes in fets:', fet1.shape[0], fet2.shape[0]

    # mean and inverted covariance of first cluster
    stepf = fet1.shape[0] / 10000
    # fet1 = fet1[range(0, fet1.shape[0], stepf),:]
    # print step
    #print fet1
    cov1 = np.cov(scipy.transpose(fet1))
    covi1 = linalg.inv(cov1)
    m1 = scipy.mean(fet1, 0)

    # distance from 2nd to 1st
    mdists = [scipy.spatial.distance.mahalanobis(fet2[i,0:fetdim], m1,covi1) for i in range(0, fet2.shape[0])]
    print 'Dists from 2nd to 1st:', mdists[:10]
    #plt.hist(mdists)
    #plt.show()

    # sort
    imsort = np.argsort(mdists)

    # number of steps to cut part of cluster, 0.25% step
    step = max(n2 / STEP, 1)
    #step = 20

    print 'Step:', step

    # iterate and count how many spikes are still in the refractory

    # initial fractions and ISI counts
    f1, i1_1, i2_1 = ref_frac(res1, refr_ms_1, 10)
    f2, i1_2, i2_2 = ref_frac(res2, refr_ms_1, 10)
    res_cut_2 = res2[:]
    res_ext_1 = res1[:]

    frac2_s = i1_1 / float(i2_1) if i2_1 > 0 else 0
    frac1_s = i1_2 / float(i2_2) if i2_2 > 0 else 0
    ref_fracs = [frac1_s]
    ref_fracs2 = [frac2_s]
    calc_limit = int(STEP*LIMPER/100.0)

    print 'Initial fractions / ISIs', f1, i1_1, i2_1, f2, i1_2, i2_2

    # to process with c++: write arrays
    f = open('clu_clean.tmp', 'w')
    f.write(str(len(res1)) + '\n')
    for a in res1:
            f.write(str(a) + '\n');
    f.write(str(len(res2)) + '\n')
    for a in res2:
            f.write(str(a) + '\n');
    for a in imsort:
            f.write(str(a) + '\n');

    use_cpp = False

    if use_cpp:
            command = 'clu_clear_couple clu_clean.tmp ' + str(refr_ms_1)
            process = subprocess.call(command.split());

            ffra = open('clu_clean.tmp.frac')
            ar = ffra.readline().split(' ')[:-1]
            for i in range(0, len(ar)/2):
                    ref_fracs2.append(float(ar[2 * i]))
                    ref_fracs.append(float(ar[2 * i + 1]))

            calc_limit = len(ref_fracs) + 1

    print 'calc_limit', calc_limit
    print 'Start moving iterations...'
    for i in range(1, 1 if use_cpp else calc_limit):
            # print 'Iteration', i
            printProgressBar(i, calc_limit, length=200)
            
            #res_cut = scipy.array([res2[j] for j in imsort[(i * step):]])
            #res_cut = scipy.sort(res_cut)
            #ref_fracs.append(ref_frac(res_cut, refr_ms_1, 10))

            res_cut = scipy.array([res2[j] for j in imsort[((i - 1) * step):(i * step)]])
            res_cut = scipy.sort(res_cut)
            res_cut_2, i1_2, i2_2, frac2 = ref_frac_inc_rem(res_cut_2, res_cut, refr_ms_1, 10, i1_2, i2_2)
            ref_fracs.append(frac2)
            
            # add spikes from clu2 to clu1
            #res2_add = res1 + [res2[j] for j in imsort[0:(i*step)]]
            #res2_add = scipy.sort(res2_add)
            #ref_fracs2.append(ref_frac(res2_add, refr_ms_1, 10))

            res2_add = scipy.array([res2[j] for j in imsort[((i-1)*step):(i*step)]])
            res2_add = scipy.sort(res2_add)
            res_ext_1, i1_1, i2_1, frac1 = ref_frac_inc(res_ext_1, res2_add, refr_ms_1, 10, i1_1, i2_1)
            ref_fracs2.append(frac1)

    # print 'Ref fracs', ref_fracs, ref_fracs2

    # was 100.0 instead of LIMPER

    fig = plt.figure()
    cid = fig.canvas.mpl_connect('button_press_event', onclick)

    xvals = scipy.array(range(0, calc_limit)) * LIMPER /calc_limit
    plt.plot(xvals, ref_fracs, label = 'Purity of the cluster being cleaned')
    plt.plot(xvals, ref_fracs2, 'g', label = 'Purity of receiver cluster')
    plt.axhline(0.01, color='r')
    plt.axhline(0.005, color='r')
    plt.show()
