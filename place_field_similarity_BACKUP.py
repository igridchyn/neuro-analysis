...
log_and_print(logdir, 'WARNING: PARTIAL NOT CALCULATED BUT FILTERING USED!')
...
c = 1
while os.path.isfile(root+'/pf_'+str(c)+'_0.mat'):
	c += 1
NCELLS = c-1
...
# 2nd batch - if not given optional parameter batch2 => is the same
rootsuf2 = gdv(op, 'batch2', '')
if rootsuf2 == '':
	root2 = root
else:
	root2 = argv[1] + rootsuf2

if root2 != root:
	c = 1
	while os.path.isfile(root2+'/pf_'+str(c)+'_0.mat'):
        	c += 1
	if c-1 < NCELLS:
		NCELLS = c-1
		cells = range(1, NCELLS+1)
...
print 'WARINIG: an occupancy filter from a different PF revision used: pfs_BS6_FTS_1'
...
for c in cells:
	...
	pf1m = genfromtxt(root + '/pf_' + str(c) + '_' + session1 + '.mat')
	pf2m = genfromtxt(root2 + '/pf_' + str(c) + '_' + session2 + '.mat')

	# print pf1m.shape, pf2m.shape

	if PART:
		# cpmtrp;
		pfcm = genfromtxt(root + '/pf_'+ str(c) + '_' + session_c + '.mat')

	# as a template for occupancy ! : pf / end learning 
	# TO BE ABLE TO USE WITH OTHER PF REVISIONS, USE SPECIFIC PATH:
	# pf_el_occ = genfromtxt(root + 'occ_5.mat')
	pf_el_occ = genfromtxt(argv[1] + 'pfs_BS6_FTS_1/pf_occ_5.mat')
	pf_el_occ1 = genfromtxt(argv[1] + 'pfs_BS6_FTS_1/pf_occ_1.mat')
	pf_el_occ2 = genfromtxt(argv[1] + 'pfs_BS6_FTS_1/pf_occ_2.mat')
	...
	# DOF - discard bins unoccupied in 2nd half of new leraning
	print 'WARNING: bins, uncoccupied in the end of nwe learning, excluded ...'
	pf1m[pf_el_occ < 1] = np.nan
	pf2m[pf_el_occ < 1] = np.nan
	#pf1m[pf_el_occ1 < 1] = np.nan
	#pf2m[pf_el_occ1 < 1] = np.nan
	#pf1m[pf_el_occ2 < 1] = np.nan
	#pf2m[pf_el_occ2 < 1] = np.nan
...
print 'Sizes of pfss_e1 and responses: %d / %d' % (len(responses), pfss_e1.shape[0])
