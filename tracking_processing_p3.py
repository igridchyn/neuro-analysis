...
def whl_to_speed(whl):
    speed = []
    SRAD = 8
    RAD_ALT = list(range(9, 50)) + [7, 6, 5]
    for i in range(0, SRAD):
        speed.append(0)
    for i in range(SRAD, len(whl)-SRAD):
        if whl[i+SRAD][0] > 0 and whl[i-SRAD][0] > 0:
            speed.append(sqrt((whl[i+SRAD][0]-whl[i-SRAD][0])**2 + (whl[i+SRAD][1]-whl[i-SRAD][1])**2))
        else:
            found = False
            for R in RAD_ALT:
                if i+R < len(whl) and i >= R and whl[i+R][0] > 0 and whl[i-R][0] > 0:
                    speed.append(sqrt((whl[i+R][0]-whl[i-R][0])**2 + (whl[i+R][1]-whl[i-R][1])**2) * SRAD/R)
                    found = True
                    break
            if not found:
                speed.append(0)
